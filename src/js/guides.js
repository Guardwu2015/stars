...
function createSpaceRadius (radius, color, representationScale) {
  ...
  representationScale = representationScale || 1
  ...
  mesh.update = function () {
    if (camera.position.z < 2.0) { this.visible = false } else
    if (camera.position.z < 800) { this.visible = true } else { this.visible = false }
  }
