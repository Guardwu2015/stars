function makeOortCloud () {
  ...
  for (var i = 0; i < positions.length; i += 3) {
    ...
    var multDistr = distrib
    if (Math.random() > 0.98) { multDistr = Math.pow(multDistr, 1 + Math.random() * 6) }
  ...
  attachLegacyMarker('Oort Cloud', oortInfo, 1.0, {
    min: 40.0,
    max: 500.0
  })
...
function makeSolarSystem () {
  ...
  attachLegacyMarker('Mercury', mercury, 1.0, {
    min: 3.4,
    max: 8.0
  })
  ...
  attachLegacyMarker('Venus', venus, 1.0, {
    min: 3.6,
    max: 8.2
  })
  ...
  earth.name = 'Earth'
  attachLegacyMarker('Earth', earth, 1.0, {
    min: 3.8,
    max: 8.4
  })
  ...
  attachLegacyMarker('Mars', mars, 1.0, {
    min: 4.0,
    max: 8.6
  })
  ...
  attachLegacyMarker('Jupiter', jupiter, 1.0, {
    min: 8.0,
    max: 17.2
  })
  ...
  attachLegacyMarker('Saturn', saturn, 1.0, {
    min: 8.0,
    max: 17.6
  })
  ...
  attachLegacyMarker('Uranus', uranus, 1.0, {
    min: 8.0,
    max: 18.0
  })
  ...
  attachLegacyMarker('Neptune', neptune, 1.0, {
    min: 8.0,
    max: 20.0
  })
  ...
  sub.position.y = 0.08
  measurement.add(sub)
  attachLegacyMarker('One light year.', sub, 1.0, {
    min: 120,
    max: 400
  })
  ...
  oortCloud.update = function () {
    if (camera.position.z > 40 && camera.position.z < 600) { this.visible = true } else { this.visible = false }
  }
