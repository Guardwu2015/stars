...
function generateGalaxy () {
  ...
  for (var i = 0; i < count; i++) {
    ...
    if (Math.random() > 0.99) { p.size *= Math.pow(1 + Math.random(), 3 + Math.random() * 3) * 0.9 } else
    if (Math.random() > 0.7) { p.size *= 1 + Math.pow(1 + Math.random(), 2) * 0.04 }
  ...
  window.toggleGalacticMeasurement = function (desired) {
    if (desired == undefined) { pGalacticSystem.measurement.visible = !this.measurement.visible } else { pGalacticSystem.measurement.visible = desired }
  }
  ...
  pGalacticSystem.update = function () {
    ...
    if (pSystem) {
      ...
      if (pDustSystem) {
        if (heatVisionValue > 0) { pDustSystem.visible = false } else { pDustSystem.visible = true }
      }
    }
