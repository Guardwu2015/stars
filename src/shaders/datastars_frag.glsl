...
void main(){
    ...
    float clampedLookup=clamp(starColorLookup,0.,1.);
    ...
    vec3 blueTarget=(vec3(1.)-vec3(.5,.6,.65))*cameraDistance;
    diffuse.xyz-=blueTarget*(1.-heatVision);
    ...
    if(heatVision>0.&&heatVisionTextureColor.w<.5){
        discard;
    }
