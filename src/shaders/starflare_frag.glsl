...
void main(){
    ...
    float intensity=1.45-dot(vNormal,vec3(0.,0.,1.))*2.;
    vec3 outerGlow=vec3(1.,1.,1.)*pow(intensity,2.);
    
    float distanceToCenter=clamp(length(screenPosition.xyz)-.45,0.,1.);
    ...
    spectralColor.xyz*=10.;
    
    // gl_FragColor = vec4( distanceToCenter, 0., 0., 1.0 );
    gl_FragColor=vec4((colorIndex-pow(intensity,2.)*.1)*pow(distanceToCenter,3.)*spectralColor.xyz,1.);
