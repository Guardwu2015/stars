...
void main(){
    ...
    vec2 uv2=vec2(vUv.x+cos(time)*.001,vUv.y+sin(time)*.001);
    vec3 secondaryColor=texture2D(texturePrimary,uv2).xyz;
    
    //	finally give it an outer rim to blow out the edges
    float intensity=1.15-dot(vNormal,vec3(0.,0.,.3));
    vec3 outerGlow=vec3(1.,.8,.6)*pow(intensity,6.);
    
    vec3 desiredColor=foundColor+outerGlow-secondaryColor;
    float darkness=1.-clamp(length(desiredColor),0.,1.);
    vec3 colorCorrection=vec3(.7,.4,.01)*pow(darkness,2.)*secondaryColor;
    desiredColor+=colorCorrection;
    
    //	the final composite color
    gl_FragColor=vec4(desiredColor,1.);
}